...
def compute_saliency_maps(model, figs, image_tensor, image_pil, methods, use_input_variance=False, variance=5):
    for method in methods:
        if method == 'CAM':
            cam_extractor = CAM(model, 'layer4', 'linear')
            with torch.no_grad():
                out = model(image_tensor.unsqueeze(0), use_input_variance, variance)
            ...
            with torch.no_grad(): 
                out = model(image_tensor.unsqueeze(0).requires_grad_(), use_input_variance, variance)
            cls_idx = out.squeeze(0).argmax().item()
            act_maps = cam_extractor(class_idx=cls_idx, scores=out)
            activation_map_fused = act_maps[0] if len(act_maps) == 1 else cam_extractor.fuse_cams(act_maps)
            figs.append(overlay_mask(image_pil, to_pil_image(activation_map_fused, mode='F'), alpha=0.5))
...
def explain(image_tensor, image_pil, methods, model_paths, device, epochs_to_compare, single_model=None, use_input_variance=False, variance=5):
    figs = []
    torch.manual_seed(1)
    ...
    if single_model is not None:
        ...
        figs = compute_saliency_maps(model, figs, image_tensor, image_pil, methods, use_input_variance, variance)
        
    else:
        for i, model_path in enumerate(model_paths):
            model = load_model(model_path, device=device)
            figs.append(get_image_from_digit(epochs_to_compare[i]))
            figs = compute_saliency_maps(model, figs, image_tensor, image_pil, methods)
